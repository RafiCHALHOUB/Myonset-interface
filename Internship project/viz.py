...
class VizUi(QtWidgets.QWidget):
    ...
    def press_key(self, eventKey):
        key = eventKey.key()
        #         print(key)
        if key == self.key_go_to_prev:
            self.go_to_prev()
        elif key == self.key_go_to_next:
            self.go_to_next()
        elif key == self.key_delete_movable:
            self.delete_movable()

    #         else: self.keyPressEvent()

    def load_data(self, data, events, code_t0, tmin=-.5, tmax=1.5, channels='all', code_movable_1='onset',
                  code_movable_2='offset', sync_chan=[[0, 1], [0, 1], [2, 3], [2, 3]], group_chan=True,
                  list_trials=None, random_order=False):
        # self.t = times
        ...
        if (self.list_epochs >= self.nb_epoch).any():
            raise ValueError(('Data contains only {} trials, can not plot trials {} .').format(self.nb_epoch,
                                                                                               self.list_epochs[
                                                                                                   self.list_epochs >= self.nb_epoch]))
        ...
        if group_chan:
            ...
            self.group_chan = [range(self.nb_chan)]
        ...
        for c in range(self.nb_chan):
            ...
            self.vmin_plot.append(self.data_epochs[:, self.sync_chan[c], :].min() - np.abs(
                self.data_epochs[:, self.sync_chan[c], :].min() * .01))
            self.vmax_plot.append(self.data_epochs[:, self.sync_chan[c], :].max() + np.abs(
                self.data_epochs[:, self.sync_chan[c], :].max() * .01))
        ...
        if any(self.movable_events.find_events(chan=-1)):
            raise ValueError(
                "Events {}: only events linked to one channel can be movable, please link each {} and {} to only one channel.".format(
                    self.movable_events.find_events(chan=-1), self.movable_1, self.movable_2))
        ...
        for group in self.group_chan:
            ...
            for c in group:
                ...
                self.event_plot[-1].setYLink(
                    self.event_plot[self.channels.index(self.sync_chan[self.channels.index(c)][0])])
    ...
    def set_time_data(self):
        ...
        self.ep_events = self.fix_events.segment(code_t0=self.code_t0, tmin=self.tmin_plot, tmax=self.tmax_plot,
                                                 print_epochs=False)
    ...
    def plot_epoch(self):
        global current_epoch
        for c in range(self.nb_chan):
            fix_chan_events = self.ep_events.list_evts_trials[self.list_epochs[current_epoch]].find_and_get_events(
                chan=[-1, self.channels[c]], print_find_evt=False)

            movable_epoch_idx = self.movable_events.find_events(chan=self.channels[c], tmin=self.ep_events.tmin.time[
                self.list_epochs[current_epoch]], tmax=self.ep_events.tmax.time[self.list_epochs[current_epoch]])
            movable_chan = self.movable_events.get_events(movable_epoch_idx)
            movable_chan.lat = Latency(
                sample=movable_chan.lat.sample - self.ep_events.tmin.sample[self.list_epochs[current_epoch]], \
                time=movable_chan.lat.time - self.ep_events.tmin.time[self.list_epochs[current_epoch]] + self.tmin_plot, \
                sf=self.sf)
            self.movable_events.del_events(movable_epoch_idx,
                                           print_del_evt=False)  # remove it before user change anything
    ...
    def get_plot_events(self, chan, movable_events=None, data_time_range=True):
        if movable_events is True:
            selected_lines = [li[0] for li in enumerate(self.event_plot[chan].list_plot_lines) if (li[1].movable)]
        elif movable_events is False:
            selected_lines = [li[0] for li in enumerate(self.event_plot[chan].list_plot_lines) if
                              (li[1].movable is False)]
        else:
            selected_lines = range(len(self.event_plot[chan].list_plot_lines))
        if data_time_range is True: selected_lines = [li for li in selected_lines if
                                                      self.t[0] <= self.event_plot[chan].get_events_pos(li) <= self.t[
                                                          -1]]
        ...
        return Events(time=evt_lat.time, sample=evt_lat.sample + self.t0sample,
                      chan=[self.channels[chan]] * len(selected_lines),
                      code=self.event_plot[chan].get_events_code(selected_lines), sf=self.sf)
    ...
    def update_movable_events(self):
        ...
        for c in range(self.nb_chan):
            ...
            plot_events.lat = Latency(
                sample=plot_events.lat.sample + self.ep_events.tmin.sample[self.list_epochs[current_epoch]], \
    ...
    def go_to_slider(self):
        global current_epoch
        if self.user_info_epoch[
            current_epoch] != self.slider.value():  # does everything only if change was made through slider (otherwise it is already done)
            self.update_movable_events()
            try:
    ...
    def set_chan_scale(self):
        try:
            self.chan_scale = self.sync_chan[int(self.combo_chan_scale.currentText())]
        except:
            self.chan_scale = range(self.nb_chan)
    ...
    def reset_scale(self):
        for c in range(self.nb_chan):
            ...
            self.vmin_plot[c] = self.data[:, self.sync_chan[c], :].min() - np.abs(
                self.data[:, self.sync_chan[c], :].min() * .01)
            self.vmax_plot[c] = self.data[:, self.sync_chan[c], :].max() + np.abs(
                self.data[:, self.sync_chan[c], :].max() * .01)
...
class VizApplication(QtWidgets.QApplication):
    ...
    def load_data(self, data, events, code_t0, tmin=-.5, tmax=1.5, channels='all', code_movable_1='onset',
        ...
        if len(data.shape) > 2:
            ...
            print(
                'Do you wish to continue and create continuous events and data from epochs (at your own risks !) ? (y)es / (n)o')
            answer = input()
        ...
        self.window.load_data(data, events, code_t0, tmin=tmin, tmax=tmax, channels=channels,
                              code_movable_1=code_movable_1, code_movable_2=code_movable_2, sync_chan=sync_chan,
                              group_chan=group_chan, list_trials=list_trials, random_order=random_order)
